...
Shader "FXLab/Glass/Refraction_Dispersion_Blurred" {
	Properties {
		...
		_BlurFactor ("Blur Factor", Float) = 10
	...
	SubShader {
		...
		void vert (inout appdata_full v, out Input o) {
			...
			float3 tan1 = mul(rotation, ((float3x3)unity_ObjectToWorld)[0].xyz)*1.0;
			float3 tan2 = mul(rotation, ((float3x3)unity_ObjectToWorld)[1].xyz)*1.0;
			float3 tan3 = mul(rotation, ((float3x3)unity_ObjectToWorld)[2].xyz)*1.0;
		...
		void surf (Input IN, inout SurfaceOutput o)
		{
			...
			refr *= sampleScreenDispersionBlurred(screenUv, screenUVOffset, dispersionFactor, _BlurFactor / 1000);
